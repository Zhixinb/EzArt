...
def compute_style_loss(style_rep, image_vgg):
    style_losses = list(map(
        tf.nn.l2_loss, [a - b for (a, b) in zip(style_rep, get_style_rep(image_vgg))]))
    style_losses = [style_losses[i] / (style_rep[i].size)
                    for i in range(len(style_losses))]
    return reduce(tf.add, style_losses)


def style_transfer(content_path, style_path, output_dir="output/", iterations=1000, vgg_path='vgg19.npy', preserve_color=True):
    ...
    with tf.Session() as sess:
        ...
        for i in range(1, iterations + 1):
            ...
            print(fmt_str.format(i, iterations, ALPHA *
                                 content_loss.eval(), BETA * style_loss.eval()))
